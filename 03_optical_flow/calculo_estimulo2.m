...
% October 2008
% Rev: Jan 2009
function calculo_estimulo(nombre_video,frame_initial,frame_final)
...
folder=['/home/users/jpgonzalezh/',nombre_video,'/'];
%cd (folder)
...
k=1;
%k=120 video Aleja
im3 = imread([folder,'muestra/',num2str(k),'.tif']);
%B1 = imrotate(im3,180);
B1=im3;
B1=B1(1:end,end:-1:1,:);
%[as]=eyes_detect_points(B1);

load(['/home/users/jpgonzalezh/',nombre_video,'/',nombre_video(end-5:end),'_ojos.mat'])
...
for k=finitial:ffinal%1000 %Fragmento del video que se va a procesar
   ...
   %im4 = read(PDvideoObj,k+1);
	...
	im3=imread([folder,'muestra/',num2str(k),'.tif']);
	im4=imread([folder,'muestra/',num2str(k+1),'.tif']);   
    im3=im3(1:end,end:-1:1,:);
    im4=im4(1:end,end:-1:1,:);
 %B1 = imrotate(im3,180);
   % B2 = imrotate(im4,180);
  ...
  im1=B1((pos(2):(pos(2)+pos(4))),(pos(1):(pos(1)+pos(3))),:);
    ...
    im1b=im2double(im1).*1;
    im2b=im2double(im2).*1;
